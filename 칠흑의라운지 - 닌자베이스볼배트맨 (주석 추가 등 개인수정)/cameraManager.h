...
public:
	...
	long getCameraX() { return cameraLocation.x; }	long getCameraY() { return cameraLocation.y; }
	// 카메라 x와 y를 바꿔줌(설정자)
	void setCameraX(long x);	void setCameraY(long y);
	// focusOnRect(RECT, x화면의 n분의1 지점까지 와야 스크롤할지, y화면의 n분의1 지점까지 와야 스크롤할지)
	void focusOnRect(RECT &rc, int x = 2, int y = 2);
	// 미니맵 자체를 그리는 함수 minimap(hdc, 생성할 x좌표, y좌표, 폭, 높이,
	// 전체맵기준으로 할지 여부 아니면 윈도우창기준)
