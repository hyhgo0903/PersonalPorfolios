...
void stageManager::render()
{
	...
	{ 
		...
		if (_isoTile[i].inRect)	IMAGEMANAGER->findImage("mapTiles")->alphaFrameRender(getMemDC(),
			_isoTile[i].drawX, _isoTile[i].drawY, _isoTile[i].fX, _isoTile[i].fY, 100);
		else IMAGEMANAGER->findImage("mapTiles")->frameRender(getMemDC(), _isoTile[i].drawX,
			_isoTile[i].drawY, _isoTile[i].fX, _isoTile[i].fY);
	}
	if (!_battlePhase)
	{
...
void stageManager::stageChange()
{
	...
	if (!_enemyFlagOn) // 상대 깃발 터뜨림
	{
		...
		case STAGE3: _score += _gold; CAMERAMANAGER->setCameraX(_score);
			SCENEMANAGER->changeScene("엔딩씬"); break;
		}
	}	
}