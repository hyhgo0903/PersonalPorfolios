...
void soundManager::addSound(string keyName, string soundName, bool bgm, bool loop)
{
	if (loop)
	{
		if (bgm)
		{
			_vBgmKey.push_back(keyName); // bgm이 true면 키값을 bgm키값 벡터에 넣어 관리
	...
	_latestAdded = soundName; // 최근 추가한 사운드 경로 및 파일명(로딩씬에서 출력)
	_mTotalSounds.insert(make_pair(keyName, &_sound[_mTotalSounds.size()])); // 키값과 페어로 맵에 넣는다
}
